...
def render_html(
    *,
    ...
    rendered = render_html_to_string(
        findings=findings, snapshot=snapshot, chain=chain, counts=counts, run_meta=run_meta
    )
    out_path.write_text(rendered, encoding="utf-8")
    return out_path


def render_html_to_string(
    *,
    findings: list[Finding],
    snapshot: TenantSnapshot,
    chain: dict[str, Any],
    ...
    """Render full report HTML and return as string."""
...
def render_exec_summary_to_string(
    *,
    findings: list[Finding],
    snapshot: TenantSnapshot,
    chain: dict[str, Any],
    ...
    """Render executive summary HTML and return as string."""
    env = Environment(loader=FileSystemLoader(str(_TEMPLATE_DIR)), autoescape=select_autoescape())
...
def render_exec_summary(
    *,
    ...
    """
    rendered = render_exec_summary_to_string(
        findings=findings, snapshot=snapshot, chain=chain, counts=counts, run_meta=run_meta
    )
    out_path.write_text(rendered, encoding="utf-8")
    return out_path
